...
## Deferred from: code review of 4-2-retry-wiring-journey-5-e2e (2026-04-29)

- Rapid double-click on `ErrorIndicator` fires duplicate retry mutations — no in-flight guard in `handleRetry`. `todo.syncStatus` is still `"failed"` on the second click before the first mutation's `onMutate` fires. Debounce or `isPending` guard would fix. Low-impact: mutations are idempotent.
- `switch (todo.failedMutation)` in `handleRetry` has no `default` case — silent no-op if `failedMutation` is `undefined`. Requires a future code path where `syncStatus: "failed"` is set without `failedMutation` to trigger; not currently possible.
